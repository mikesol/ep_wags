...
exports.stopMediaRecorder = (mr) => () => {
  if (mr.state !== "inactive") {
    mr.stop();
  } else {
    console.error("Recorder is not active, cannot stop.");
  }
};
exports.mediaRecorderToBlob2 = function (mimeType) {
  return function (handler) {
    return function (mrHack) {
      return function (mediaRecorder) {
        return function () {
          mrHack(mediaRecorder)();
          var chunks = [];
...
exports.uploadBlbToFilestackAndSendMsgToChat = (blob) => () => {
  const client = filestack.init("ArcQnSXbqT4O0OORDyYxrz");
  client.upload(blob).then((data) => {
    const text = "Recording has been uploaded to: " + data.url;
    sendMessage(text);
  });
};
...
exports.postToolbarInit_ = (args) => (cb) => () => {
  ...
  editbar.registerCommand("epWagsExport", () => {
    if (exportOn.io) {
      $editBar
        .find(".ep-wags-export-on")
        .removeClass("ep-wags-export-on")
        .addClass("ep-wags-export-off");
      exportOn.io = false;
    } else {
      $editBar
        .find(".ep-wags-export-off")
        .removeClass("ep-wags-export-off")
        .addClass("ep-wags-export-on");
      exportOn.io = true;
    }
  });
